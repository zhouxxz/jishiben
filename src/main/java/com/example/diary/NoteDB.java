...
public class NoteDB {
    ...
    public void update_db(String title,String author,String text,String time,int item_ID){//改
        if( text.isEmpty()){
            Toast.makeText(context, "各字段不能为空", Toast.LENGTH_LONG).show();
        }
        else{
            db.execSQL("update notes set context='"+text+ "',title='"+title+"',author='"+author+"',time='"+time+"'where _id='" + item_ID+"'");
        }
    }
    ...
    public void insert_db(String title,String author,String text,String time){
        if(text.isEmpty()){
            Toast.makeText(context, "各字段不能为空", Toast.LENGTH_LONG).show();
        }
        else{
            db.execSQL("insert into notes(title,author,context,time) values('"+ title+"','"+ author+ "','"+ text+ "','"+time+"');");
        }
    }
